    ...
    if (!errorPresent) {
      if (email.current.value === emailConf.current.value) {
        if (password.current.value === passwordConf.current.value) {
          ...
          try {
            const response = await axios.post('http://localhost:3000/api/usermodif', { newInfos });
            ...
            if (response.data.success) {
              ...
              console.log("L'user que je reçois est : ", response.data.user);
              // Mise à jour du ceontext
              setUserData(response.data.user);
              console.log("Mon nouveau context est : ", userData);

    
              navigate( `/esn/${response.data.userId}`);
            }
            else {
              setGeneralError("Modification échouée");
            }
        ...
        >
          ...
          <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 3 }}>
            ...
            <Button
              ...
              >
              { isSubmitting ? 'Envoi en cours...' : 'Mettre à jour mes informations' }
            </Button>
